        ...
        for iteration in range(self.max_iterations):
            ...
            for i in range(len(trainingData)):
                ...
                for direction in {'North', ' East', 'West', 'South'}:
                    if trainingData[i][0][direction] != 0:
                        counter[direction] = self.weights * \
                            trainingData[i][0][direction]

                if trainingLabels[i] != counter.argMax():
                    self.weights += trainingData[i][0][trainingLabels[i]]
                    self.weights -= trainingData[i][0][counter.argMax()]
